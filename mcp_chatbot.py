...
class MCP_ChatBot:
    ...
    async def connect_to_server(self, server_config: ServerConfig):
        try:
            params = server_config.config.model_dump()
            if server_config.type == "sse":
                client = sse_client(**params)
            elif server_config.type == "stdio":
                stdio_params = StdioServerParameters(**params)
                client = stdio_client(stdio_params)
            else:
            ...
            try:
                ...
                print(f"Error listing tools for {server_config.name}: {e}")
            ...
            print(f"\nConnected to {server_config.name} with tools: {tools}")
            if resources:
            ...
            print(f"Error connecting to {server_config.name}: {e}")
    ...
    async def connect_to_servers(self):
        try:
            ...
            for server_config in servers:
                server_config_model = ServerConfig(**server_config)
                await self.connect_to_server(server_config_model)
        except Exception as e:
        ...
        while response.stop_reason == 'tool_use':
            ...
            for block in response.content:
                if block.type == 'text':
                    ...
                    if not session:
                        print(f"Tool '{block.name}' not found.")
                        tool_results.append({
                            'type': 'tool_result',
